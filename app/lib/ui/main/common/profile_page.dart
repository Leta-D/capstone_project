...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void initState() {
    ...
    if (context.read<ProfileControllerCubit>().state
        is ProfileControllerInitialState) {
      context.read<ProfileControllerCubit>().fetchProfile();
    }
    super.initState();
  }
    ...
    return SingleChildScrollView(
      child: BlocBuilder<ProfileControllerCubit, ProfileControllerState>(
        builder: (context, state) {
          if (state is ProfileControllerInitialState) {
            context.read<ProfileControllerCubit>().fetchProfile();
          }
          if (state is ProfileControllerLoadingState) {
            return CircularProgressIndicator();
          }
          if (state is ProfileControllerLoadedState) {
            nameController.text = state.role == "user"
                ? state.name
                : "Hope For All Foundation";
            emailController.text = state.role == "user"
                ? state.email
                : "contact@hopeforall.org";
                ...
                Center(
                  child: Column(
                    children: [
                      ...
                      Container(
                        decoration: BoxDecoration(
                          color: AppColors.lightGreen(0.2),
                        ),
                        child: Column(
                          children: [
                            Padding(
          ...
          if (state is ProfileControllerErrorState) {
